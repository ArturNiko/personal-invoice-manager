    ...
    try {
        ...
        if (res.status === 422) {
            ...
            if (!Object.keys(errors.value).length) {
                generalError.value = data.message ?? 'Invalid credentials.'
            }
            return
        }
        ...
        <div class="w-full max-w-md rounded-3xl border border-white/10 bg-white/5 p-6 shadow-2xl shadow-slate-950/50 backdrop-blur-xl sm:p-8">
            ...
            <form @submit.prevent="submit" class="space-y-5">
                ...
                <label class="flex items-center gap-2 text-sm text-slate-300">
                    <input v-model="remember" type="checkbox" class="h-4 w-4 rounded border-white/20 bg-slate-900 text-cyan-400 focus:ring-cyan-500" />
                    Remember me
                </label>
            ...
            <div class="mt-6 text-center text-sm text-slate-300">
                Need an account?
                <RouterLink to="/register" class="font-semibold text-cyan-300 hover:text-cyan-200">
                    Create one
                </RouterLink>
            </div>
        </div>
